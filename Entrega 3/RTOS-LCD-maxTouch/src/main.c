...
static void config_AFEC_pot(Afec *afec, uint32_t afec_id, uint32_t afec_channel, afec_callback_t callback){
  /*************************************
  * Activates and configures AFEC
  *************************************/
  /* Activates AFEC - 0 */
  afec_enable(afec);

  /* AFEC configuration struct */
  ...
  /*** Specific configuration of the AFEC channel ***/
...
uint32_t convert_axis_system_x(uint32_t touch_y) {
  // input: 4096 - 0 (current coordinate system)
  // output: 0 - 320
  return ILI9488_LCD_WIDTH - ILI9488_LCD_WIDTH*touch_y/4096;
}

uint32_t convert_axis_system_y(uint32_t touch_x) {
  // input: 0 - 4096 (current coordinate system)
  // output: 0 - 320
...
void task_lcd(void){
  ...
  // Starts the LCD and paints it white
  configure_lcd();
  draw_screen();
     
  // Local strut to store msg sent by mxt task
  touchData touch;
  adcData adc;
  ...
  while (true) {
	...
	// Finds a new value in the adc queue, formats and prints the data on the LCD
	if (xQueueReceiveFromISR( xQueueADC, &(adc), ( TickType_t )  100 / portTICK_PERIOD_MS)) {
		...
		ili9488_draw_rectangle(10, 100, 310, 150);
		ili9488_set_foreground_color(COLOR_CONVERT(COLOR_WHITE));
		ili9488_draw_filled_rectangle(12, 105, 308, 145);
		int q = adc.value/340;
		for(int i = 0; i < q; i++){
			if(i < 5){
			...
			ili9488_draw_filled_circle(26 + (27*i), 125, 10);
		}
	}
  }
}
...
void task_adc(void){
	...
	/* Starts and configures the adc */
	config_AFEC_pot(AFEC_POT, AFEC_POT_ID, AFEC_POT_CHANNEL, AFEC_pot_Callback);

	/* Selects channel and initializes conversion */
