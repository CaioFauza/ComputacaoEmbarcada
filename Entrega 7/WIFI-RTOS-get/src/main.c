...
SemaphoreHandle_t xSemaphore, xSemaphore_1;
...
void get_formatter(uint8_t *g_sendBuffer, char endpoint[]){
...
static void task_process(void *pvParameters) {
  ...
  while(1){
	   if( xSemaphoreTake(xSemaphore_1, ( TickType_t ) 500) == pdTRUE ){
		   pio_get_output_data_status(LED_PIO, LED_IDX_MASK) ? pio_clear(LED_PIO, LED_IDX_MASK) : pio_set(LED_PIO, LED_IDX_MASK);
	   }
    switch(state){
      case WAIT:
      ...
      case GET:
      printf("STATE: GET \n");
	  get_formatter(g_sendBuffer, "/status");
      ...
      if(xQueueReceive(xQueueMsg, &p_recvMsg, 5000) == pdTRUE){
        ...
        printf(p_recvMsg->pu8Buffer);
        printf(STRING_EOL);  printf(STRING_LINE);
		led_status = strstr(p_recvMsg->pu8Buffer, "led");
		! strcmp(led_status[7], '1') ? pio_clear(LED_PIO, LED_IDX_MASK) : pio_set(LED_PIO, LED_IDX_MASK);
		state = DONE;
      }
      else {
        state = TIMEOUT;
